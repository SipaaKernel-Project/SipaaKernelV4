...
namespace SipaaKernel.Graphics
{
	...
	public unsafe class FrameBuffer : IDisposable
	{
		...
		public FrameBuffer Scale(uint Width, uint Height, ScaleMode Mode = ScaleMode.Normal)
		{
			switch (Mode)
			{
				#region DontKeep
				case ScaleMode.DontKeep:
					Internal = (uint*)HeapEx.Realloc((byte*)Internal, Size * 4);
					this.Width = Width;
					this.Height = Height;
					return this;
				#endregion
		...
		/// <summary>
		/// Update the FPS, frames and the total frames
		/// </summary>
		public void UpdateFPS()
        {
			if (_Delta != Cosmos.HAL.RTC.Second)
            {
				_Delta = Cosmos.HAL.RTC.Second;
				_FPS = _Frames;
	...
	{
		/// <summary>
		/// Don't keep data in the buffer (Fast)
		/// </summary>
		DontKeep,
		/// <summary>
		/// Normal mode (Recomended for images)
		/// </summary>
		Normal,
	}
}